...
export const MosaicSlide = memo(({ posts }: MosaicSlideProps) => {
    ...
    return (
        <div className="absolute inset-0 bg-surface z-40 p-4 animate-fade-in flex items-center justify-center">
            {/* Background Decor */}
             <div className="absolute inset-0 bg-black opacity-90 z-0"></div>
            <div className="absolute inset-0 bg-surface opacity-90 z-0"></div>
            ...
            <div className="z-10 grid grid-cols-4 md:grid-cols-4 gap-4 w-full h-full p-8 max-w-[1920px] mx-auto">
                {displayPosts.map((post, index) => {
                    ...
                    return (
                        <div 
                            key={post.id} 
                            className={`relative rounded-lg overflow-hidden shadow-lg border border-border/10 aspect-square transform transition-all duration-700 animate-fade-in-up ${delayClass}`}
                        >
                            {/* Mosaic Image */}
                            <img 
                            ...
                            <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent p-2 text-white text-xs opacity-0 hover:opacity-100 transition-opacity">
                                                                <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-surface/80 to-transparent p-2 text-foreground text-xs opacity-0 hover:opacity-100 transition-opacity">
             ...
             <div className="absolute bottom-8 right-8 z-20 bg-black/60 backdrop-blur-md px-6 py-2 rounded-full border border-white/10 animate-pulse">
                                <div className="absolute bottom-8 right-8 z-20 bg-surface/60 backdrop-blur-md px-6 py-2 rounded-full border border-border/10 animate-pulse">
                                    <span className="text-foreground font-bold tracking-widest uppercase text-sm">Mural do Evento</span>
                                </div>
             </div>
        </div>
    );
